...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row % 2 == 0) {
        ...
        static NSDictionary *stringAttributes = nil;
        static dispatch_once_t onceToken;
        dispatch_once(&onceToken, ^{
            stringAttributes = @{ NSFontAttributeName : [UIFont systemFontOfSize:17]};
        });
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    switch (indexPath.section) {
        ...
        case 3:
            //visit website
            if (indexPath.row == 1) {
                TOWebViewController *webBrowser = [[TOWebViewController alloc] initWithURLString:@"http://m.rutgers.edu/ruinfo.html"];
